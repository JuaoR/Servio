...
export function useComandas(restaurantId:string,ownerName:string,caixaAtiva:CaixaSessao|null,setMovimentacoesCaixa:React.Dispatch<React.SetStateAction<MovimentacaoCaixa[]>>,setHistory:React.Dispatch<React.SetStateAction<HistoricoItem[]>>){
 ...
 useEffect(()=>{if(!restaurantId){setComandas(makeEmptyComandas());return;}let cancelled=false;const employee=isEmployeeSession();
  ...
  const channel=supabase.channel('admin-comandas-'+restaurantId).on('postgres_changes',{event:'*',schema:'public',table:'comandas',filter:'restaurant_id=eq.'+restaurantId},async payload=>{
    ...
    if(saveQueues.current[number])return;const {data}=await supabase.from('comandas').select('*, comanda_items(*)').eq('id',row.id).maybeSingle();if(data&&!cancelled){const mapped=mapRows([data])[number];if(mapped)setComandas(prev=>({...prev,[number]:mapped}));}
  }).on('postgres_changes',{event:'*',schema:'public',table:'comanda_items'},async payload=>{
    if(cancelled)return;const comandaId=(payload.new as any)?.comanda_id||(payload.old as any)?.comanda_id;if(!comandaId)return;const parent=(await supabase.from('comandas').select('id,number,status').eq('id',comandaId).eq('restaurant_id',restaurantId).maybeSingle()).data;if(!parent||parent.status!=='aberta'||saveQueues.current[Number(parent.number)])return;const {data}=await supabase.from('comandas').select('*, comanda_items(*)').eq('id',comandaId).maybeSingle();if(data&&!cancelled){const mapped=mapRows([data])[Number(data.number)];if(mapped)setComandas(prev=>({...prev,[Number(data.number)]:mapped}));}
  }).subscribe();
 ...
 const handleOpenComanda=async(id:number)=>{if(!restaurantId)return;if(isEmployeeSession()){const token=localStorage.getItem('servio_emp_token');if(!token)return;const {data,error}=await supabase.rpc('employee_open_comanda',{p_token:token,p_number:id});if(error){alert('Erro ao abrir comanda: '+error.message);return;}setComandas(prev=>({...prev,[id]:{...prev[id],id,uuid:data?.uuid,status:'aberta',openedAt:Number(data?.openedAt||Date.now()),mesa:data?.mesa||'',obs:data?.obs||'',discount:Number(data?.discount||0)}}));return;}
  const {data,error}=await supabase.from('comandas').select('*, comanda_items(*)').eq('restaurant_id',restaurantId).eq('number',id).eq('status','aberta').maybeSingle();if(error){console.error('[handleOpenComanda]',error);return;}if(data&&(data.comanda_items||[]).length>0)setComandas(prev=>({...prev,[id]:mapRows([data])[id]}));
 };
 ...
 const handleItemsUpdate=async(id:number,items:ItemPedido[],discount=0)=>{if(!restaurantId)return;if(isEmployeeSession()){const current=comandas[id];setComandas(prev=>({...prev,[id]:{...prev[id],items,discount,status:items.length?'aberta':prev[id].status}}));const token=localStorage.getItem('servio_emp_token');if(!token)return;const {data,error}=await supabase.rpc('employee_save_comanda',{p_token:token,p_number:id,p_items:items,p_discount:discount,p_mesa:current?.mesa||'',p_obs:current?.obs||''});if(error){console.error('[employee_save_comanda]',error);return;}setComandas(prev=>({...prev,[id]:{...prev[id],uuid:data?.uuid,status:'aberta'}}));return;}
  // Optimistic UI: the click is reflected immediately, while persistence is serialized per comanda.
  ...
  const job=previous.catch(()=>undefined).then(async()=>{
    // Always resolve the canonical DB row HERE, after previous writes have finished.
    // Never use the React closure state for the UUID; rapid clicks may arrive before state updates.
    ...
    if(items.length===0){
      ...
      uuid=existing?.id;
      if(uuid)await supabase.from('comandas').delete().eq('id',uuid).eq('restaurant_id',restaurantId).eq('status','aberta');
      setComandas(prev=>({...prev,[id]:{...prev[id],status:'livre',items:[],uuid:undefined,openedAt:null,discount:0}}));
      return;
    }
    ...
    if(!uuid){
      ...
      if(error){
        if((error as any).code==='23505'){
          const {data:db}=await supabase.from('comandas').select('id,opened_at').eq('restaurant_id',restaurantId).eq('number',id).eq('status','aberta').maybeSingle();
          uuid=db?.id;
          if(db?.opened_at)setComandas(prev=>({...prev,[id]:{...prev[id],uuid,status:'aberta',openedAt:new Date(db.opened_at).getTime()}}));
        } else { console.error('[handleItemsUpdate] create:',error); alert('Erro ao salvar comanda: '+mapSupabaseError(error)); return; }
      } else {
        uuid=nova?.id;
        if(nova?.opened_at)setComandas(prev=>({...prev,[id]:{...prev[id],uuid,status:'aberta',openedAt:new Date(nova.opened_at).getTime()}}));
      }
    }
    if(!uuid)return;

    const subtotal=items.reduce((s,it)=>s+it.price*it.qty,0),total=Math.max(0,subtotal-discount);
    const {error:deleteError}=await supabase.from('comanda_items').delete().eq('comanda_id',uuid);
    if(deleteError){console.error('[handleItemsUpdate] delete items:',deleteError);return;}
    const {error:insertError}=await supabase.from('comanda_items').insert(items.map(it=>({comanda_id:uuid!,product_id:it.pid,name:it.name,price:it.price,quantity:it.qty,notes:it.note||null})));
    if(insertError){console.error('[handleItemsUpdate] insert items:',insertError);return;}
    const {error:updateError}=await supabase.from('comandas').update({discount,subtotal,total,updated_at:new Date().toISOString()}).eq('id',uuid).eq('restaurant_id',restaurantId);
    if(updateError){console.error('[handleItemsUpdate] update comanda:',updateError);return;}
    setComandas(prev=>({...prev,[id]:{...prev[id],uuid,status:'aberta',items,discount,openedAt:prev[id].openedAt||Date.now()}}));
  });
 ...
 const handleCloseEmptyComanda=async(id:number)=>{if(isEmployeeSession()){setComandas(prev=>({...prev,[id]:{...prev[id],status:'livre',openedAt:null}}));return;}const uuid=comandas[id]?.uuid;if(uuid)await supabase.from('comandas').delete().eq('id',uuid).eq('restaurant_id',restaurantId).eq('status','aberta');setComandas(prev=>({...prev,[id]:{...prev[id],status:'livre',items:[],uuid:undefined,openedAt:null,discount:0}}));};
