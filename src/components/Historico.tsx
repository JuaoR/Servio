...
import { HistoricoItem, Categoria, Produto, Funcionario } from '../types';
...
export default function Historico({ history, categories, products, onClearHistory, funcionarios = [] }: HistoricoProps) {
  ...
  const waiterAnalytics = allWaiterNames.map(name => {
    ...
    const waiterReg = funcionarios.find(g => g.name.toLowerCase() === name.toLowerCase());
    ...
    const code = waiterReg ? waiterReg.id.substring(0, 4) : '—';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex bg-[var(--bg-card)] border border-[var(--border-color)] p-1 rounded-xl w-full sm:w-fit">
        ...
        >
          <Users size={14} />
          <span>Desempenho dos Funcionários</span>
        </button>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-5 border-l-4 border-l-amber-500">
              ...
              <h3 className="text-2xl font-black text-[var(--text-main)] mt-1">{formatCurrency(filteredRevenue)}</h3>
              <p className="text-xs text-[var(--text-muted)] mt-2">{recs.length} comanda(s) encontradas</p>
            </div>

            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-5 border-l-4 border-l-sky-500">
              <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Ticket Médio (Filtrado)</p>
              <h3 className="text-2xl font-black text-[var(--text-main)] mt-1">{formatCurrency(filteredTicket)}</h3>
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-5 border-l-4 border-l-sky-500">
              ...
              <h3 className="text-2xl font-black text-[var(--text-main)] mt-1">{formatCurrency(overallRevenue)}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="flex flex-wrap gap-2.5">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      className="bg-[var(--bg-base)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-xs text-[var(--text-muted)] outline-none focus:border-sky-500 font-sans"
                    />
                  </div>

                  {/* Waiter */}
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Funcionário</span>
                    <input
                      type="text"
                      placeholder="Nome do funcionário..."
                      value={filterWaiter}
                      onChange={(e) => setFilterWaiter(e.target.value)}
                      className="bg-[var(--bg-base)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-xs text-[var(--text-muted)] placeholder-zinc-400 outline-none focus:border-sky-500"
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      ...
                      className="bg-[var(--bg-base)] border border-[var(--border-color)] rounded-lg px-3 py-1.5 text-xs text-[var(--text-muted)] outline-none focus:border-sky-500"
                    >
                  ...
                  {chartDays.map((d, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center h-full group relative">
                        {/* Val popup */}
                        {d.value > 0 && (
                          <div className="absolute top-[-24px] opacity-0 group-hover:opacity-100 transition-opacity bg-[var(--bg-panel)] border border-[var(--border-color)] px-2 py-0.5 rounded text-[10px] text-[var(--text-main)] font-bold whitespace-nowrap z-10 shadow-lg">
                            R$ {d.value.toFixed(0)}
                          </div>
                        )}
                        ...
                        <div className="w-full flex-1 flex items-end">
                          <div
                            className={`w-full rounded-t-md transition-all duration-500 relative cursor-pointer ${
                              d.isToday
                                ? 'bg-sky-500 opacity-90 group-hover:opacity-100'
                  ...
                  <table className="w-full text-left text-sm border-collapse">
                    <thead>
                      <tr className="border-b border-[var(--border-color)] bg-[#FAF7F2]">
                        ...
                        <th className="py-3 px-4 text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Funcionário</th>
                        ...
                        recs.map(h => {
                          ...
                          return (
                            <tr key={h.id} className="border-b border-[#EAE3D5] hover:bg-[#FAF7F2] transition-colors">
                              <td className="py-3 px-4 font-bold text-[var(--text-muted)]">#{h.cmdId}</td>
                              ...
                              <td className="py-3 px-4">
                                <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2 py-0.5 rounded-full bg-[var(--bg-base)] border border-[var(--border-color)] text-[var(--text-muted)]">
                                  <span>{pay.ic}</span>
                                  <span>{pay.lb}</span>
                                </span>
                              </td>
                              ...
                              <td className="py-3 px-4 font-extrabold text-[var(--text-main)]">{formatCurrency(h.total)}</td>
                              <td className="py-3 px-4 text-right">
                                <button
                                  onClick={() => setSelectedReceipt(h)}
                                  className="px-2.5 py-1 bg-zinc-100 hover:bg-zinc-200 text-xs font-bold text-[var(--text-muted)] rounded-md border border-[var(--border-color)] transition-all cursor-pointer inline-flex items-center gap-1"
                                >
                                  <Eye size={12} />
                                  <span>Ver</span>
                                </button>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-4">
                <h4 className="text-xs font-bold text-[var(--text-muted)] mb-3 flex items-center gap-2 pb-2 border-b border-[var(--border-color)]">
                  ...
                  <div className="space-y-2">
                    {topProducts.map(([name, val], i) => (
                      <div key={name} className="flex justify-between items-center text-xs">
                        ...
                        <span className="font-bold text-[var(--text-main)]">{formatCurrency(val)}</span>
                      </div>
                    ))}
                  </div>
                )}
                    ...
                    {Object.entries(paymentRevenue).map(([key, val]) => {
                      ...
                      return (
                        <div key={key} className="flex justify-between items-center text-xs">
                          ...
                          <span className="font-extrabold text-[var(--text-muted)]">{formatCurrency(val as number)}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-4 space-y-2">
                ...
                <button
                  onClick={handleExportCSV}
                  className="w-full py-2 bg-zinc-100 border border-[var(--border-color)] text-[var(--text-muted)] hover:bg-zinc-200 text-xs font-semibold rounded-lg cursor-pointer flex items-center justify-center gap-1.5"
                >
                  <Download size={13} />
      ...
      {/* TAB 2: DETALHAMENTO DE FUNCIONÁRIOS (ESTILO CONSUMER) */}
      {activeTab === 'garcons' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-5 border-l-4 border-l-sky-500">
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Total de Comissões Devidas</span>
              <h3 className="text-2xl font-black text-[var(--text-main)] mt-1">{formatCurrency(totalStaffCommissions)}</h3>
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-5 border-l-4 border-l-sky-500">
              ...
              <h3 className="text-2xl font-black text-[var(--text-main)] mt-1">
                {formatCurrency(waiterAnalytics.length > 0 ? totalStaffCommissions / waiterAnalytics.length : 0)}
              </h3>
              <p className="text-[11px] text-[var(--text-muted)] mt-1.5">Média por funcionário ativo</p>
            </div>
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-5 border-l-4 border-l-blue-500 relative overflow-hidden">
              ...
              {bestWaiter ? (
                <>
                  <h3 className="text-lg font-black text-[var(--text-muted)] mt-1 truncate">{bestWaiter.name}</h3>
                  <p className="text-[11px] text-[var(--text-muted)] mt-0.5">Vendeu <strong className="text-sky-500">{formatCurrency(bestWaiter.totalSales)}</strong></p>
                </>
              ) : (
                <p className="text-xs text-[var(--text-muted)] mt-2">Nenhum lançamento registrado</p>
              )}
              <Trophy size={45} className="absolute right-2 bottom-1 text-sky-500/10 shrink-0" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-5 space-y-4">
              <div className="flex justify-between items-center pb-3 border-b border-[var(--border-color)]">
                <div>
                  <h3 className="text-sm font-bold text-[var(--text-muted)] flex items-center gap-1.5">
                    <Trophy className="text-sky-500" size={16} />
                    <span>Ranking de Vendas por Funcionário</span>
                  </h3>
              ...
              {waiterAnalytics.length === 0 ? (
                <p className="text-xs text-center py-12 text-[var(--text-muted)]">Nenhum lançamento no histórico associado a funcionários.</p>
              ) : (
                        ...
                        <div className="flex justify-between items-center text-xs">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`w-5 h-5 rounded-full flex items-center justify-center font-bold text-[10px] ${
                              index === 0 ? 'bg-sky-500 text-[var(--text-main)]' : index === 1 ? 'bg-zinc-300 text-zinc-800' : index === 2 ? 'bg-sky-700 text-[var(--text-main)]' : 'bg-zinc-100 text-zinc-500'
                            }`}>
                              {index + 1}
                            </span>
                            <span className="font-bold text-[var(--text-muted)]">{w.name}</span>
                            <span className="text-[9px] text-[var(--text-muted)] font-mono">({w.orderCount} mesas)</span>
                          </div>
                          
                          <div className="text-right">
                            <span className="font-black text-[var(--text-muted)] font-mono">{formatCurrency(w.totalSales)}</span>
                        ...
                        <div className="w-full h-2.5 bg-zinc-100 border border-zinc-200/50 rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full transition-all duration-700 ${
                              index === 0 ? 'bg-gradient-to-r from-sky-500 to-sky-600' : index === 1 ? 'bg-gradient-to-r from-purple-500 to-purple-600' : 'bg-gradient-to-r from-emerald-500 to-emerald-600'
                            }`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-5 flex flex-col justify-between h-full">
              <div>
                <h4 className="text-xs font-bold text-[var(--text-muted)] pb-2 border-b border-[var(--border-color)] uppercase tracking-wider flex items-center gap-1.5">
                ...
                <div className="space-y-4 pt-4">
                  <div>
                    <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1.5">Selecione o Funcionário</label>
                    <select
                      value={selectedWaiterDetail}
                      onChange={(e) => setSelectedWaiterDetail(e.target.value)}
                      className="w-full p-2 bg-[var(--bg-base)] border border-[var(--border-color)] rounded-xl text-xs text-[var(--text-muted)]"
                    >
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden p-5 space-y-4">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 pb-3 border-b border-[var(--border-color)]">
              <div>
                <h3 className="text-sm font-bold text-[var(--text-muted)] flex items-center gap-1.5">
                  <UserCheck className="text-sky-500" size={16} />
                  <span>Livro de Lançamentos por Funcionário</span>
                </h3>
                ...
                <tbody>
                  {selectedWaiterData.length === 0 ? (
                    ...
                    selectedWaiterData.map(h => {
                      ...
                      const matchedGarcom = funcionarios.find(g => g.name.toLowerCase() === h.garcom.toLowerCase());
                      ...
                      return (
                        <tr key={h.id} className="border-b border-[#EAE3D5] hover:bg-[#FAF7F2] transition-colors">
                          ...
                          <td className="py-3 px-4 font-bold text-[var(--text-muted)]">Mesa / Comanda #{h.cmdId} {h.mesa ? `(${h.mesa})` : ''}</td>
                          <td className="py-3 px-4 text-[var(--text-muted)] font-medium">{h.garcom || '—'}</td>
                          ...
                          <td className="py-3 px-4 text-right font-extrabold text-sky-600">{formatCurrency(orderCommission)}</td>
                          <td className="py-3 px-4 text-right">
                            <button
                              onClick={() => setSelectedReceipt(h)}
                              className="px-2 py-0.5 bg-zinc-100 hover:bg-zinc-200 border border-[var(--border-color)] text-[10px] font-bold text-[var(--text-muted)] rounded-md transition-all cursor-pointer"
                            >
                              Detalhar
                            </button>
                          </td>
      ...
      {selectedReceipt && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <motion.div
            ...
            className="w-full max-w-sm bg-[var(--bg-base)] border border-[var(--border-color)] rounded-xl overflow-hidden shadow-2xl flex flex-col"
          >
            <div className="p-4 border-b border-[var(--border-color)] flex justify-between items-center bg-[#FAF7F2]">
              <div>
                <h3 className="font-bold text-[var(--text-muted)] text-sm">Detalhamento Comanda #{selectedReceipt.cmdId}</h3>
            ...
            <div className="p-5 overflow-y-auto max-h-[60vh] space-y-4">
              <div className="grid grid-cols-2 gap-3 text-xs bg-zinc-50 border border-[#EAE3D5] p-3 rounded-lg">
                <div>
                  ...
                  <span className="text-[var(--text-muted)] font-semibold">{selectedReceipt.mesa || '—'}</span>
                </div>
                <div>
                  <span className="block text-[10px] text-zinc-500 font-bold uppercase">Funcionário</span>
                  <span className="text-[var(--text-muted)] font-semibold">{selectedReceipt.garcom || '—'}</span>
                </div>
                <div className="col-span-2 pt-2 border-t border-[#EAE3D5] mt-1">
                  ...
                  <span className="inline-flex items-center gap-1 mt-0.5 font-bold text-sky-600">
              ...
              {selectedReceipt.obs && (
                <div className="p-2.5 bg-[#FAF7F2] text-xs text-sky-600 rounded border border-sky-500/10">
              ...
              <div className="space-y-1.5">
                ...
                {selectedReceipt.items.map((it, idx) => (
                  <div key={idx} className="flex justify-between text-xs py-1 border-b border-[#FAF7F2] text-[var(--text-muted)]">
                    <div>
                      <span>{it.qty}x {it.name}</span>
                      {it.note && <span className="block text-[10px] text-sky-600">↳ Obs: {it.note}</span>}
                    </div>
              ...
              <div className="pt-2 border-t border-[var(--border-color)] text-xs space-y-1.5 font-sans">
                ...
                <div className="flex justify-between text-base font-extrabold text-[var(--text-main)] pt-2 border-t border-[var(--border-color)] mt-1.5">
            ...
            <div className="p-4 border-t border-[var(--border-color)] bg-[#FAF7F2] flex justify-end">
              <button
                onClick={() => setSelectedReceipt(null)}
                className="px-4 py-2 bg-[var(--bg-panel)] hover:bg-black text-[var(--text-main)] text-xs font-bold rounded-lg cursor-pointer"
              >
                Fechar
              </button>
            </div>
      ...
      <ConfirmModal
        ...
        message="Tem certeza de que deseja apagar TODO o histórico de vendas permanentemente?"
        confirmText="Sim, Apagar Histórico"
        onConfirm={() => {
          onClearHistory();
