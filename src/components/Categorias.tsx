...
export default function Categorias({ categories, products, onCreateCategory, onUpdateCategory, onDeleteCategory }: CategoriasProps) {
  ...
  const [categoryToDelete, setCategoryToDelete] = useState<{id: string, count: number} | null>(null);
  ...
  const handleDelete = (id: string) => {
    ...
    setCategoryToDelete({ id, count: productCount });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <ConfirmModal
        ...
        message={categoryToDelete?.count && categoryToDelete.count > 0 
          ? `Esta categoria contém ${categoryToDelete.count} produto(s). Excluir mesmo assim? Todos os produtos vinculados perderão a referência de categoria.` 
